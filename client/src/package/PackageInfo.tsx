...
import { TableContainer, Paper, Table, TableHead, TableRow, TableCell, TableBody, Avatar, Dialog, DialogTitle, List, ListItem, ListItemAvatar, ListItemButton, ListItemText, TextField, DialogContent } from "@mui/material";
...
const PackageInfo = () => {
  const { id } = useParams();
  const [packageDetails, setPackageDetails] = useState<PackageInfoProps | null>(null);
          ...
          {packageDetails ? (
            ...
            <div>
              <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                ...
                <Grid item xs={10}>
                  <p className="package-item-grid">{packageDetails.description}</p>
                </Grid>
                <Grid item xs={2} sx={{fontWeight: 'bold'}}>
                ...
                <Grid item xs={10}>
                  <Link href={packageDetails.publication}>
                    <p className="package-item-grid">{packageDetails.publication}</p>
                  </Link>
                </Grid>
                <Grid item xs={2} sx={{fontWeight: 'bold'}}>
      ...
      <TableContainer component={Paper}>
        <Table size="small" stickyHeader>
          <TableHead sx = {{
            ...
            <TableRow>
              <TableCell sx={{textAlign: 'center'}}>Available versions</TableCell>
            ...
            {versions.map((verItem) => (
                <TableRow key={verItem.version} hover={true}>
                  ...
                  <TableCell sx={{textAlign: 'center'}}>
                    <IconButton aria-label="download url" onClick={() => clickDownloadVersionHandler(verItem)}>
                      <CloudDownloadIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
...
const DownloadDialog = (props: SimpleDialogProps) => {
  const { onClose, selectedVersion, open } = props;
    ...
    <Dialog onClose={handleClose} open={open} sx={{ m: 0, p: 2 }}>
      ...
      <DialogContent sx={{textAlign: 'center'}}>
        <TextField
          ...
          />
          <Button variant="contained" startIcon={<CloudDownloadIcon />} disabled={isDisabled} href={'https://drive.google.com/uc?export=download&id=13TNNeM0iQ8yowv3Ow0wZZSXgnKnwMG6b'}>
            Download
          </Button>
      </DialogContent>
      </Dialog>
